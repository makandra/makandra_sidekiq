...
describe MakandraSidekiq::SidekiqControl do
  ...
  describe '#quiet' do
    ...
    it 'runs "sidekiqctl quiet" in a new environment' do
      expect(subject).to receive(:with_original_env).and_yield do |scope|
  ...
  describe '#with_original_env' do
    it 'delegates to Bundler.with_original_env' do
      block = Proc.new {}
      expect(Bundler).to(receive(:with_original_env)) { |&given_block| expect(given_block).to eq(block) }
      subject.send(:with_original_env, &block)
    end
  end
